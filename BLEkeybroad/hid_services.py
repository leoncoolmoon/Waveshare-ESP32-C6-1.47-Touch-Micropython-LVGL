...
class Mouse(HumanInterfaceDevice):
    def __init__(self, name="Bluetooth Mouse"):
        ...
        self.HID_INPUT_REPORT = [                                                                                       # Report Description: describes what we communicate.
            ...
            0x0A, 0x38, 0x02,                                                                                           #         Usage (AC Pan) - horizontal scroll, 16-bit usage 0x0238
            0x15, 0x81,                                                                                                 #         Logical Minimum (-127)
            0x25, 0x7F,                                                                                                 #         Logical Maximum (127)
            0x75, 0x08,                                                                                                 #         Report Size (8)
            0x95, 0x01,                                                                                                 #         Report Count (1)
            0x81, 0x06,                                                                                                 #         Input(Data, Variable, Relative); horizontal wheel byte
            0xc0,                                                                                                       #   END_COLLECTION
            0xc0                                                                                                        # END_COLLECTION
        ]
        # fmt: on
        ...
        self.x = 0
        self.y = 0
        self.w = 0                                                                                                       # Vertical wheel.
        self.p = 0                                                                                                       # Horizontal wheel (AC Pan).
    ...
    def save_service_characteristics(self, handles):
        ...
        state = struct.pack("Bbbbb", b, self.x, self.y, self.w, self.p)                                                 # Pack the initial mouse state as described by the input report (buttons, X, Y, vertical wheel, horizontal wheel).
    ...
    def notify_hid_report(self):
        if self.is_connected():
            ...
            state = struct.pack("Bbbbb", b, self.x, self.y, self.w, self.p)                                             # Pack the mouse state as described by the input report (buttons, X, Y, vertical wheel, horizontal wheel).
            self.characteristics[self.h_rep] = ("HID report", state)
            self._ble.gatts_notify(self.conn_handle, self.h_rep, state)                                                 # Notify client by writing to the report handle.
    ...
    # Set the mouse scroll wheel value (vertical scroll).
...
# 在 hid_services.py 末尾添加

class ComboHID(HumanInterfaceDevice):
    """合并键盘和鼠标功能的HID设备 - 直接使用HumanInterfaceDevice基类"""
    
    def __init__(self, name="BLE Combo HID"):
        super(ComboHID, self).__init__(name)
        self.device_appearance = 962  # Mouse appearance
        ...
        self.HIDS = (
            ...
            (
                ...
                # 键盘输出 (Report ID 1)
                (UUID(0x2A4D), F_READ_WRITE_NOTIFY_NORESPONSE, (
                    (UUID(0x2908), DSC_F_READ),
                )),
                # 鼠标输入 (Report ID 2)
                (UUID(0x2A4D), F_READ_NOTIFY, (
        ...
        self._kb_output_callback = None
        self.services.append(self.HIDS)
    
    # ========== 生命周期 ==========
    def start(self):
        super(ComboHID, self).start()
        handles = self._ble.gatts_register_services(self.services)
        self._save_characteristics(handles)
        self.write_service_characteristics()
        self.adv = Advertiser(self._ble, [UUID(0x1812)], self.device_appearance, self.device_name)
        print("Combo HID (Keyboard + Mouse) started")
    
    def _save_characteristics(self, handles):
        super(ComboHID, self).save_service_characteristics(handles)
        
        (h_info, h_hid, h_ctrl, 
         self._h_kb_in, h_kb_ref1, 
         self._h_kb_out, h_kb_ref2,
         self._h_mouse_in, h_mouse_ref,
         h_proto) = handles[3]
        
        # 键盘初始状态
        kb_state = struct.pack("8B", self._modifiers, 0, 
                               self._keycodes[0], self._keycodes[1],
                               self._keycodes[2], self._keycodes[3],
                               self._keycodes[4], self._keycodes[5])
        
        # 鼠标初始状态
        mouse_state = struct.pack("Bbbbb", self._mouse_btn, 
                                  self._mouse_x, self._mouse_y, 
                                  self._mouse_w, self._mouse_p)
        
        self.characteristics[h_info] = ("HID info", b"\x01\x01\x00\x00")
        self.characteristics[h_hid] = ("HID report map", bytes(self.HID_INPUT_REPORT))
        self.characteristics[h_ctrl] = ("HID control point", b"\x00")
        
        self.characteristics[self._h_kb_in] = ("Keyboard input", kb_state)
        self.characteristics[h_kb_ref1] = ("KB input ref", struct.pack("<BB", 1, 1))
        self.characteristics[self._h_kb_out] = ("Keyboard output", kb_state)
        self.characteristics[h_kb_ref2] = ("KB output ref", struct.pack("<BB", 1, 2))
        
        self.characteristics[self._h_mouse_in] = ("Mouse input", mouse_state)
        self.characteristics[h_mouse_ref] = ("Mouse input ref", struct.pack("<BB", 2, 1))
        self.characteristics[h_proto] = ("HID protocol", b"\x01")
    
    def ble_irq(self, event, data):
        if event == _IRQ_GATTS_WRITE:
            conn_handle, attr_handle = data
            if attr_handle == self._h_kb_out:
                report = self._ble.gatts_read(attr_handle)
                if self._kb_output_callback:
                    self._kb_output_callback(struct.unpack("8B", report))
                return _GATTS_NO_ERROR
        return super(ComboHID, self).ble_irq(event, data)
    
    # ========== 键盘API ==========
    def kb_set_modifiers(self, left_control=0, left_shift=0, left_alt=0, left_gui=0,
                        right_control=0, right_shift=0, right_alt=0, right_gui=0):
        self._modifiers = (right_gui << 7) | (right_alt << 6) | (right_shift << 5) | \
                         (right_control << 4) | (left_gui << 3) | (left_alt << 2) | \
                         (left_shift << 1) | left_control
    
    def kb_set_keys(self, k0=0, k1=0, k2=0, k3=0, k4=0, k5=0):
        self._keycodes = [k0, k1, k2, k3, k4, k5]
    
    def kb_send(self):
        """发送键盘报告"""
        if self.is_connected():
            state = struct.pack("8B", self._modifiers, 0,
                               self._keycodes[0], self._keycodes[1],
                               self._keycodes[2], self._keycodes[3],
                               self._keycodes[4], self._keycodes[5])
            self._ble.gatts_notify(self.conn_handle, self._h_kb_in, state)
    
    def kb_press(self, key_code, modifiers=0):
        """按下并释放一个键"""
        if not self.is_connected():
            return
        self.kb_set_modifiers(
        ...
        self.kb_set_keys(key_code, 0, 0, 0, 0, 0)
        self.kb_send()
        import time
        time.sleep_ms(20)
        self.kb_set_modifiers()
    ...
    def kb_set_output_callback(self, callback):
        """设置键盘输出回调（接收LED状态等）"""
        self._kb_output_callback = callback
    
    # ========== 鼠标API ==========
    def mouse_set_xy(self, x=0, y=0):
        self._mouse_x = max(-127, min(127, int(x)))
        self._mouse_y = max(-127, min(127, int(y)))
    
    def mouse_set_wheel(self, w=0):
        self._mouse_w = max(-127, min(127, int(w)))
    
    def mouse_set_pan(self, p=0):
        self._mouse_p = max(-127, min(127, int(p)))
    
    def mouse_set_buttons(self, left=0, right=0, middle=0):
        self._mouse_btn = (1 if left else 0) | (2 if right else 0) | (4 if middle else 0)
    
    def mouse_send(self):
        """发送鼠标报告"""
        if self.is_connected():
            state = struct.pack("Bbbbb", self._mouse_btn,
                               self._mouse_x, self._mouse_y,
                               self._mouse_w, self._mouse_p)
            self._ble.gatts_notify(self.conn_handle, self._h_mouse_in, state)
    
    def mouse_move(self, dx, dy):
        """移动鼠标（相对位移）"""
        self.mouse_set_xy(dx, dy)
        self.mouse_send()
        import time
        time.sleep_ms(5)
        self.mouse_set_xy(0, 0)
        self.mouse_send()
    
    def mouse_scroll(self, dy, dx=0):
        """滚动滚轮"""
        self.mouse_set_wheel(dy)
        self.mouse_set_pan(dx)
        self.mouse_send()
        import time
        time.sleep_ms(5)
        self.mouse_set_wheel(0)
        self.mouse_set_pan(0)
        self.mouse_send()
    
    def mouse_click(self, button='left'):
        """点击鼠标按钮"""
        btn_map = {'left': (1,0,0), 'right': (0,1,0), 'middle': (0,0,1)}
        b = btn_map.get(button, (1,0,0))
        self.mouse_set_buttons(*b)
        self.mouse_send()
        import time
        time.sleep_ms(50)
        self.mouse_set_buttons(0, 0, 0)
        self.mouse_send()