...
class MouseUI:
    ...
    def _create_touch_panel(self):
        """触摸拖动区域。之前这里额外套了一个 lv.canvas 只是为了填充背景色，
        但canvas需要额外分配一整块RGB565像素缓冲区(这里是172x~120x2字节，约几万字节)，
        纯粹浪费内存还容易在反复创建/销毁UI后造成堆碎片化，导致MemoryError。
        实际上背景色 panel 自己(lv.obj)就能直接用样式画出来，不需要canvas，
        触摸拖动的坐标计算也完全是几何运算，跟这里有没有canvas控件毫无关系。"""
    ...
    def handle_mouse_scroll(self, move_dx, move_dy):
        ...
        if abs(self.scroll_accum_y) >= abs(self.scroll_accum_x):
            ...
            if getattr(self.cfg, 'invert_vscroll', False):
                notches = -notches
            if notches != 0:
            ...
            if getattr(self.cfg, 'invert_hscroll', False):
                notches = -notches
            if notches != 0:
        ...
        if state == indev.PRESSED:
            ...
            if 0 <= canvas_x < self.canvas_width and 0 <= canvas_y < self.canvas_height:
                if not self.touch_pressed:
                    ...
                    if getattr(self.cfg, 'swap_xy', False):
                        dx, dy = -dy, dx
