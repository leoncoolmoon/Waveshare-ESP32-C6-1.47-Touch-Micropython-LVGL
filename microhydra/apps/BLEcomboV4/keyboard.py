...
# 这几个组合键在macOS上应该用Cmd(GUI, 0x08)而不是Ctrl(0x01)，
# 由 Settings 里的 "macOS Shortcuts" 开关(cfg.os_mode)决定。
OS_AWARE_COMBOS = {'Udo', 'Cut', 'Copy', 'Paste'}
_GUI_MOD = 0x08
...
class KeyboardHID:
    ...
    def send_combo(self, key, modifiers=0, extra_mod_override=None):
        if key not in COMBO_KEYS or not self.ble.connected:
            return False

        extra_mod, target = COMBO_KEYS[key]
        if extra_mod_override is not None:
            extra_mod = extra_mod_override
...
class KeyboardUI:
    ...
    def __init__(self, scrn, keyboard_hid, cfg, width, height, initial_caps_lock=False):
        self.scrn = scrn
        ...
        self.caps_lock_on = initial_caps_lock

        self.drag_x = 290
        self.drag_width = 30
        self.preview_x = 5
        self.preview_width = 280
        self.preview_strip_height = 30
        # 预览窗口(两行叠在一起的整块)会跟着手指动态换位:
        # 手指在屏幕上半部分时，预览显示在下半部分(不被手指挡住)；
        # 手指在屏幕下半部分时，预览显示在上半部分。
        self.preview_y_upper_pos = 15   # 手指在下半屏时，预览显示在这里(屏幕上方)
        self.preview_y_lower_pos = 110  # 手指在上半屏时，预览显示在这里(屏幕下方)
        self.preview_top_y = self.preview_y_upper_pos      # 初始位置，创建控件时用
        self.preview_bottom_y = self.preview_top_y + self.preview_strip_height
        self._preview_base_y = None  # 当前实际生效的预览位置，第一次触摸时才会真正设置
    ...
    def _update_preview_position(self, touch_y):
        """预览窗口跟随手指动态换位: 手指在屏幕上半部分 -> 预览挪到下半部分；
        手指在屏幕下半部分 -> 预览挪到上半部分，避免被手指挡住。"""
        base_y = self.preview_y_lower_pos if touch_y < self.kb_vert_mid else self.preview_y_upper_pos
        if base_y == self._preview_base_y:
            return
        self._preview_base_y = base_y
        self.preview_top_y = base_y
        self.preview_bottom_y = base_y + self.preview_strip_height
        self.preview_top_container.set_pos(self.preview_x, self.preview_top_y)
        self.preview_bottom_container.set_pos(self.preview_x, self.preview_bottom_y)
    ...
    def set_caps_lock(self, on):
        """由主机端真实的键盘LED状态驱动(通过BLE keyboard output report回传)，
        而不是本地猜测/本地翻转。main.py收到host的output report后会调用这个方法。"""
        self.caps_lock_on = bool(on)
        self._update_caps_indicator()
    ...
    def kb_send_key(self, key):
        ...
        if key == 'CAP':
            # 不再本地猜测/翻转状态；真实的大写锁定状态由主机通过
            # BLE keyboard output report回传，main.py收到后会调用 set_caps_lock() 更新指示灯
        ...
        if key in COMBO_KEYS:
            extra_mod_override = None
            if key in OS_AWARE_COMBOS and getattr(self.cfg, 'os_mode', 'win') == 'mac':
                extra_mod_override = _GUI_MOD
            self.hid.send_combo(key, self._get_modifier_mask(), extra_mod_override=extra_mod_override)
            self.kb_auto_release_modifiers()
            self.kb_update_lock_badges()
            return
    ...
    def update_touch(self, indev):
        ...
        if state == indev.PRESSED:
            ...
            self._update_preview_position(y)
            ...
            if self.kb_state == 0:
                ...
                if x >= self.drag_x:
                    ...
                    col = _col_from_x(x, self.preview_x, self.preview_width, GROUP_SIZE)
                    # 上半屏选上排字符，下半屏选下排字符，直接看当前触点的位置，
                    # 不再依赖手势移动方向
                    bias = 'top' if y < self.kb_vert_mid else 'bottom'
                    if col != self.kb_col or bias != self.kb_vert_bias:
                        self.kb_col = col
                        self.kb_vert_bias = bias
                        self.kb_highlight_col(col, self.kb_vert_bias)
