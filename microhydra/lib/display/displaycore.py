...
class DisplayCore:
    ...
    def set_brightness(self, brightness: int):
        ...
        print(f"brt = {brightness}")
        _MAX_BRIGHT = const(65535)
        _MIN_BRIGHT = const(20000)
        _BRIGHT_STEP = const((_MAX_BRIGHT - _MIN_BRIGHT) // 10)
        self.brightness = _MAX_BRIGHT if brightness == 10 else _MIN_BRIGHT + _BRIGHT_STEP * brightness
        self.backlight.duty_u16(self.brightness)

    def set_backlight(self,switch:bool):
        self.backlight.duty_u16(self.brightness if switch else 0)
