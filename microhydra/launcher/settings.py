...
_TRANS = const("""[
  ...
  {"en": "sleep_after", "zh": "超时息屏", "ja": "画面消灯"}
...
def update_config(caller, value):
    ...
    if caller.text == 'brightness':
        # Map brightness 1-10 to PWM duty cycle (0-100%)
        # 1 = 10%, 10 = 100%
        brightness_percent = (value / 10) * 100
        display._display.brightness(int(brightness_percent))
...
def import_config(caller):  # noqa: ARG001
    ...
    try:
        ...
        # Apply brightness from imported config
        if 'brightness' in config.config:
            brightness_percent = (config.config['brightness'] / 10) * 100
            display._display.brightness(int(brightness_percent))
...
def build_menu() -> hydramenu.Menu:
    ...
    menu_def = [
        ...
        (hydramenu.IntItem, 'brightness', {'min_int': 1, 'max_int': 10, 'instant_callback': update_config}),
        (hydramenu.IntItem, 'sleep_after', {'min_int': 5, 'max_int': 90, 'instant_callback': update_config}),
